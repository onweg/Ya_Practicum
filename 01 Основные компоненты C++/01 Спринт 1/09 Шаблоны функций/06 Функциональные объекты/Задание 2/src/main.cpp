...
void SortBy(Container& container, KeyMapper key_mapper, bool reverse = false) {
    sort(container.begin(), container.end(), [key_mapper, reverse](const auto& lhs, const auto& rhs){return reverse ? key_mapper(lhs) > key_mapper(rhs) : key_mapper(lhs) < key_mapper(rhs);});
}
